...
class _HomePageState extends State<HomePage> with SingleTickerProviderStateMixin {
    ...
    _controller.addListener(() {
      setState(() {
        ...
        if (isAnalyzing) {
          ...
          final totalRows = 6;
          final rowHeight =
              (mappedArea.northeast.latitude - mappedArea.southwest.latitude) / totalRows;
  ...
  Future<void> _toggleMapping() async {
    if (hasMapped && !isMapping && !isAnalyzing) {
      final confirm = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          title: const Text('New Mapping?', style: TextStyle(color: Colors.white)),
          content: const Text('A mapping already exists. Start a new one?',
    ...
    if (!isMapping && !isAnalyzing) {
      ...
      Navigator.push(context, MaterialPageRoute(builder: (context) => const MappingPage()));
    } else {
      setState(() {
        _controller.stop();
        isMapping = false;
    ...
    final result = await showModalBottomSheet<String>(
      ...
      builder: (context) => Column(mainAxisSize: MainAxisSize.min, children: [
        ListTile(
            ...
            title: const Text('Auto Mode', style: TextStyle(color: Colors.white)),
            onTap: () => Navigator.pop(context, 'Auto')),
        ListTile(
            leading: const Icon(Icons.flight, color: Colors.white),
            title: const Text('Manual Mode', style: TextStyle(color: Colors.white)),
          ...
          if (isAverage)
            Align(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  color: isDarkMode ? Colors.white24 : Colors.black.withOpacity(0.06),
                  borderRadius: BorderRadius.circular(8),
                ),
                child: const Text(
                  'AVERAGE',
                        ...
                        Expanded(
                          child: ElevatedButton.icon(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor:
                                  isMapping || isAnalyzing ? Colors.redAccent : Colors.lightGreenAccent,
                    ...
                    ElevatedButton(
                      onPressed: () {
                        Navigator.push(context,
                            MaterialPageRoute(builder: (context) => const SoilMonitoringPage()));
                      },
                      style: ElevatedButton.styleFrom(
